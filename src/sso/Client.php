 ...
 *
 * The client lives on the website visited by the user. The broken doesn't have any user credentials stored. Instead it
...
class Client
{
     ...
     * @param string $client My identifier, given by SSO provider.
     * @param string $secret My secret word, given by SSO provider.
     */
    public function __construct($url, $client, $secret, $cookie_lifetime = 3600)
    {
        if (!$url) throw new \InvalidArgumentException("SSO server URL not specified");
        if (!$client) throw new \InvalidArgumentException("SSO client id not specified");
        if (!$secret) throw new \InvalidArgumentException("SSO client secret not specified");
     ...
     * Get the cookie name.
     *
     * Note: Using the client name in the cookie name.
     * This resolves issues when multiple clients are on the same domain.
     *
     * @return string
     */
    protected function getCookieName()
    {
        return 'sso_token_' . preg_replace('/[_\W]+/', '_', strtolower($this->client));
    }
    ...
    protected function getSessionId()
    {
        ...
        return "SSO-{$this->client}-{$this->token}-$checksum";
    }
     ...
     *
     * Only clients marked trused can collect and send the user's credentials. Other clients should omit $username and
     * $password.
     *
